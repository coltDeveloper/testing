  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const classData = Object.values(uniqueClasses);
        ...
        const tasksLength =
          classData.reduce(
            (count, classItem) =>
              count +
              classItem.subjects.reduce((subjectCount, subject) =>
                subject.schedules.some(schedule => schedule.day === currentDay) ? subjectCount + 1 : subjectCount
              , 0),
            0
          ) +
          examData.filter(
