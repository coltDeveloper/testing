...
const AddLessonPlan = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    } else if (documentFile.some(file => file.size > 50 * 1024 * 1024) ) {
      message.warning("File size should not be greater than 50 MB.");
    }
    else if (videoFile.some(file => file.size > 50 * 1024 * 1024)) {
      message.warning("Video size should not be greater than 50 MB.");
    }
    else {
      setLoading(true);
      try {
        ...
        const payload = {
          ...
          video: [
            ...(video || []),
            ...(lessonPlanData.videoLinks ? lessonPlanData.videoLinks.split(',').map(link => link.trim()) : [])
          ],
        };
        console.log(payload);
    ...
    <>
      <div className="bg-white pt-4 shadow-lg rounded-4 min-vh-82">
        ...
        <form className="p-3">
          ...
          <div className={`col-md-12 ${isArabic === "sa" ? 'text-end' : 'text-start'}`}>
            <label htmlFor="videoLinks" className="weakform-text form-label">
              {t("VideoLinks")}
            </label>
            <div className="mb-3">
              <input
                ...
                type="text"
                className={`form-control ${isArabic === "sa" ? 'text-end' : 'text-start'}`}
                value={lessonPlanData.videoLinks}
                onChange={(e) => setLessonPlanData({ ...lessonPlanData, videoLinks: e.target.value })}
              />
            </div>
          </div>
